...
The `:dispose` clause is now optional when your context doesn't need any cleanup logic. The macro still emits the `[Symbol.asyncDispose]` stub (with no statements) so the generated class satisfies `AsyncDisposable`. Use a bare `defcontext` without `:dispose` when the slots themselves don't own resources, for example:

```t2-lang
(defcontext SimpleContext
  (:slot read (object))
  (:slot write (object)))
```
